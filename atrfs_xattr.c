...
{
	return secs_to_time(get_dvalue(get_real_file_name(ent), "user.length", 0.0));
}

static char *get_watchtime(struct atrfs_entry *ent)
{
	return secs_to_time(get_dvalue(get_real_file_name(ent), "user.watchtime", 0.0));
}

static char *get_count(struct atrfs_entry *ent)
{
	static char buf[10]; //XXX
	sprintf(buf, "%d", get_ivalue(get_real_file_name(ent), "user.count", 0));
	return buf;
}
